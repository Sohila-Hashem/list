...
export default class ListTabulator<Renderer extends ListRenderer> {
  ...
  public pasteHandler(element: PasteEvent['detail']['data']): ListData {
    console.log("🚀 ~ ListTabulator ~ pasteHandler ~ element:", element)
    const { tagName: tag } = element;
    console.log("🚀 ~ ListTabulator ~ pasteHandler ~ tag:", tag)
    ...
    const getPastedItems = (parent: Element, level = 0): ListItem[] => {
      console.log("🚀 ~ ListTabulator ~ getPastedItems ~ level:", level)
      console.log("🚀 ~ ListTabulator ~ getPastedItems ~ parent:", parent)
      // get first level li elements.
      const children = Array.from(parent.querySelectorAll(`:scope > li`));
      console.log("🚀 ~ ListTabulator ~ getPastedItems ~ children:", children)

      return children.map((child, index) => {
        console.log("🚀 ~ ListTabulator ~ getPastedItems ~ child index:", index)
        // get text content of the li element.
        let content = '';
        Array.from(child.childNodes).forEach((node, index) => {
          console.log("🚀 ~ ListTabulator ~ getPastedItems ~ child node index:", index)
          console.log("🚀 ~ ListTabulator ~ getPastedItems ~ node:", node)
          if (node.nodeType === Node.TEXT_NODE) {
            console.log("IS A TEXT NODE");
            content += node.textContent?.trim();
          } else if (node.nodeType === Node.ELEMENT_NODE) {
            console.log("IS AN ELEMENT NODE");
            const { tagName } = node as Element;
            if (tagName === tagToSearch) {
              getPastedItems(node as Element, level + 1);
            } else {
        ...
        console.log("🚀 ~ ListTabulator ~ getPastedItems ~ content:", content)
        // get subitems if they exist.
        const subItemsWrapper = child.querySelector(`:scope > ${tagToSearch}`);
        console.log("🚀 ~ ListTabulator ~ getPastedItems ~ subItemsWrapper:", subItemsWrapper)
        // get subitems.
        const subItems = subItemsWrapper ? getPastedItems(subItemsWrapper, level + 1) : [];
        console.log("🚀 ~ ListTabulator ~ getPastedItems ~ subItems:", subItems)
        return {
          content,
          meta: {},
          items: subItems,
